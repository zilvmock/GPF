<div class="">
    @foreach($rooms as $room)
        <div id="room-{{$room->id}}"
             class="sm:flex p-2 my-2 items-center content-center justify-between max-w-full bg-white rounded-lg border border-gray-200 shadow-md
            ...
            <div class="sm:flex items-center">
                <span class="flex flex-col justify-items-center items-center w-12 mr-4
                    ...
                    <small class="text-center w-12">
                        {{$room->users_count}} / {{$room->size}}
                    </small>
                </span>
                <h1 id="room-{{$room->id}}-title"
                    class="sm:pt-0 py-2 sm:w-11/12 sm:mr-4 md:text-lg text-sm font-bold break-all tracking-tight text-gray-900 dark:text-white">
                    {{$room->title}}
                </h1>
            </div>
